...
| F (núcleo) ✅ | Ejecución determinística + snapshot/resume | Moat | **Hecho.** Modo determinista (`Capabilities.Deterministic`/`Seed`; `Randomize` seedeable). Snapshot/resume completo del estado del VM (globals, locals, pila de operandos, pila de llamadas con PCs, heap con grafos de punteros/listas enlazadas, RNG, excepciones) a bytes portables, con tabla de identidad de celdas para preservar aliasing y descubrimiento de celdas huérfanas (`New`). API durable en vmpas (`Suspend`, `RunDurable`/`ResumeDurable`, `State`) con guard de *fingerprint*. Docs (docs/durable.md) + ejemplo (examples/durable). Alcance v1: no concurrente, sin archivos abiertos. | L/XL | E6 |
| F (opcional) ◐ | Sandbox probable | Moat | **Parcial.** Gas determinista: ya cubierto por `MaxSteps`/`MaxHeap`/`MaxOutput`/`MaxCallDepth` (deterministas en modo `Deterministic`). **G1 ✅ inferencia de capacidad mínima** (`Engine.Analyze` escanea el bytecode y reporta qué capacidades usa). **G2 ✅ traza auditable** (`Capabilities.Audit` + `Engine.AuditLog`: registra cada llamada gateada en orden). Notas futuras: **G3** allowlists finas (HTTP por dominio, FS por path, SQL por verbo) y **G4** "probable" formal (certificado verificable de capacidad máxima) — diferidas salvo necesidad concreta. | M/L | F-núcleo, E4 |
